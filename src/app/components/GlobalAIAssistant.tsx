...
interface GlobalAIAssistantProps {
  currentContext?: string; // e.g., "primary-dashboard", "text-summarizer", etc.
  userName?: string;
...
export function GlobalAIAssistant({ currentContext, userName, studentType }: GlobalAIAssistantProps) {
  ...
  const getContextualSystemMessage = () => {
    ...
    if (studentType === 'primary') {
      context += `This is a primary student, so use simple, fun, and encouraging language with emojis. `;
    } else if (studentType === 'scholar') {
      context += `This is a scholar student, so use professional and detailed explanations. `;
    }

    switch (currentContext) {
      case 'primary-dashboard':
        context += `The user is on the Primary Dashboard. Help them navigate features like pattern games, memory games, reading helper, and fun lessons.`;
        break;
      case 'scholar-dashboard':
        context += `The user is on the Scholar Dashboard. Assist with text summarization, doubt clearing, quiz generation, and adaptive lessons.`;
        break;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if ((window as any).todaiSpeak && studentType === 'primary') {
        // Only auto-speak for primary students to keep it fun
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: `I'm sorry, I encountered an error: ${error.message}. Please check your API key in settings.`,
        timestamp: new Date(),
      };
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-white/80">
                      {studentType === 'primary' ? 'Your Fun Learning Buddy! 🌟' : 'Your Learning Companion'}
                    </div>
                  </div>
                </div>
            ...
            <div className="p-4 bg-white border-t border-gray-200">
              <div className="flex gap-2">
                ...
                <input
                  ...
                  placeholder={studentType === 'primary' ? 'Ask me anything! 🌟' : 'Ask me anything...'}
